...
export default class RoomFooter extends React.Component<RoomFooterProps, {}> {
    ...
    _buildRoomFooter(room: RoomModel): JSX.Element {
        ...
        if (room.items.length === 0) {
            items = <div>{i18nService.Instance.translate(TranslationConstants.noObjectsCloseBy)}</div>;
        } else {
            items = this._buildFooterLine(i18nService.Instance.translate(TranslationConstants.thereIs), room.items);
        }
        if (room.enemies.length === 0) {
            enemies = <div>{i18nService.Instance.translate(TranslationConstants.noEnemiesCloseBy)}</div>;
        } else {
            enemies = this._buildFooterLine(i18nService.Instance.translate(TranslationConstants.youSee), room.enemies.map(enemy => enemy.profile));
        }
        if (room.npc.length > 0) {
            npcs = <div>
                        {room.npc.map((npc: NPCModel, index: number) => <div key={'NPC_' + index}>
                                                            <span className={style.room__footer__npc}>{npc.id}</span> 
                                                            {i18nService.Instance.translate(TranslationConstants.isHere)}
                                                        </div>)}
                    </div>;
        }
        if (room.players.length > 0) {
            players = <div>
                        {room.players.map((player: PlayerModel, index: number) => <div key={'PLAYER_' + index}>
                                                            <span className={style.room__footer__player}>{player.id}</span> 
                                                            {i18nService.Instance.translate(TranslationConstants.isHere)}
                                                        </div>)}
                    </div>;
        }
