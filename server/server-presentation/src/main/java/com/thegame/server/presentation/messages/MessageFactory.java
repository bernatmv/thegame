...
public class MessageFactory {
	...
	private static final Map<Class<IsMessage>,Function<IsMessage,IsMessageBean>> toBeanMapping=new HashMap<>(20);
	...
	static {
		try {
			...
			Stream.of(MessageMapper.class.getDeclaredMethods())
					...
					.filter(method -> IsMessageBean.class.isAssignableFrom(method.getReturnType()))
					...
					.map(method -> new Object[]{
												...
												(Function)((messageBean) -> {
															...
															IsMessageBean reply=null;
													
															try {
																reply=(IsMessageBean)method.invoke(MessageFactory.messageMapper,messageBean);
					...
					.forEach(array -> MessageFactory.toBeanMapping.put((Class<IsMessage>)array[0],(Function<IsMessage,IsMessageBean>)array[1]));
	...
	@SuppressWarnings("element-type-mismatch")
	public static IsMessageBean getInstance(final IsMessage _message){
		
		IsMessageBean reply;
		
		logger.trace("message-factory::get-instance::IsMessage::begin::{}",_message);
		final Function<IsMessage,IsMessageBean> mapper=MessageFactory.toBeanMapping.get(_message.getClass());
