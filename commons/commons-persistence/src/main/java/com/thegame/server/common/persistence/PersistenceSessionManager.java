...
public class PersistenceSessionManager implements Closeable{
	...
	private final static LogStream logger=LogStream.getLogger(PersistenceSessionManager.class,"persistence-session-manager::");
	...
	private EntityManagerFactory createFactory(final String _persistenceUnit,final Map<String,String> _properties){
		...
		try{
			...
			logger.warning("create-factory::{}::with-properties::{}::failed",_persistenceUnit,_properties,e);
	...
	private PersistenceSession createLocalSession(){
		...
		logger.trace("register::local-session::{}",this.underlayingSessionThreadLocal.get());
	...
	public void removeSession() {
		logger.trace("unregister::local-session::{}",this.underlayingSessionThreadLocal.get());
